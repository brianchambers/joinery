...
//        }
        ...
        for (int c = 0; c < df.size(); c++) {
            if (!columns.contains(c)) {
                final List<V> column = new ArrayList<>();
                Function function = aggregateMap.getOrDefault(df.columns().toArray()[c], new Aggregate<V, Number>() {
                    @Override
                    public Number apply(List<V> values, int colIndex) {
                        return Number.class.cast(values.get(0));
                    }
        ...
        if (newcols.size() <= columns.size()) {
            throw new IllegalArgumentException(
                    "no results for provided aggregation map"
            );
        }
